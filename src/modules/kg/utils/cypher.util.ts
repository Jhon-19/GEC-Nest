...
export function operaEditNode(node, attrs) {
  ...
  if (labels) {
    return [
      `MATCH (n) WHERE id(n) = ${
        node[SYSTEM_NODE_ID]
      } REMOVE n:${oldLabels.join(',n:')}, ${oldAttrs.join(',')}`,
      `MATCH (n) WHERE id(n) = ${node[SYSTEM_NODE_ID]} SET n:${labels.join(
        ':',
      )}, ${attrMapToCypher.join(',')}`,
    ];
  } else {
    return [
      `MATCH (n) WHERE id(n) = ${
...
export function operaEditRelation(info) {
  if ('label' in info) {
    return [
      `MATCH ()-[r]->() WHERE id(r) = ${info[SYSTEM_NODE_ID]} DELETE r`,
      `MATCH (m),(n) WHERE id(m) = ${info.from} AND id(n) = ${info.to} CREATE (m)-[r:${info.label} { ${SYSTEM_RELATION_COMMENT}: '${info[SYSTEM_RELATION_COMMENT]}' }]->(n)`,
    ];
  } else {
    return [
      `MATCH ()-[r]->() WHERE id(r) = ${info[SYSTEM_NODE_ID]} SET r.${SYSTEM_RELATION_COMMENT} = '${info[SYSTEM_RELATION_COMMENT]}'`,
    ];
  }
}
